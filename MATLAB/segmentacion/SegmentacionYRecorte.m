...
drawnow;
caption = sprintf('imagen original que muestra 6 monedas grandes y 4 más pequeñas.');
...
subplot(3, 3, 2);
bar(pixelCount);
title('Histograma de la imagen original', 'FontSize', captionFontSize);
...
subplot(3, 3, 3);
imshow(binaryImage); 
title('Imagen binaria, obtenida del umbralizado', 'FontSize', captionFontSize); 
...
title('imagen etiquetada, desde bwlabel()', 'FontSize', captionFontSize);
...
axis image; % Make sure image is not artificially stretched because of screen's aspect ratio.
caption = sprintf('Imagen pseudo colorida desde label2rgb().');
title(caption, 'FontSize', captionFontSize);
...
subplot(3, 3, 6);
imshow(originalImage);
title('Lineas de contorno, desde bwboundaries()', 'FontSize', captionFontSize); 
axis image; % Make sure image is not artificially stretched because of screen's aspect ratio.
hold on;
...
axis image;
title('Monedas pequeñas separadas y clasificadas', 'FontSize', captionFontSize);
...
axis image;
title('Solo las monedas pequeñas de la imagen principal', 'FontSize', captionFontSize);
...
axis image;
title('Solo las monedas grandes de la imagen principal', 'FontSize', captionFontSize);
elapsedTime = toc;
...
if strcmpi(reply, 'Yes')
	...
	for k = 1 : numberOfBlobs           % Loop through all blobs.
		...
		subplot(3, 4, k);
		imshow(subImage);
		caption = sprintf('Moneda #%d es una %s.\nDiameter = %.1f pixels\nArea = %d pixels', ...
